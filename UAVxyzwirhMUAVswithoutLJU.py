...
import numpy as np
import calculationswithoutMUAVs as cal
import torch
...
class CustomEnv(Env):
    ...
    def reset1obs(self, copy_guinfo, LSUPosHistory, odhistoryFull):
    ...
    def step1(self, action, copy_guinfo, LSUPosHistory,odhistoryFull ):
        ...
        for key in actualGU.keys():
            secrate,r_g_s, r_g_e = cal.secrecyRate([actualGU[key][0], actualGU[key][1]],gunewtx, lsunewstate )
        ...
        terminated = False
        return state1obs, reward, terminated, False, {},copy_guinfo, LSUPosHistory,odhistoryFull, secrecyrate_info, r_g_s_info, r_g_e_info
...
def train():
    ...
    while time_step <= max_training_timesteps:
        ...
        for _ in range(8):
            odhistoryFull.extend(odhistory)
        # gutxHistory = [0 for _ in range(NumGUs * 8)]
        state = env.reset1obs(guinfo_copy, LSUPosHistory, odhistoryFull)
        current_ep_reward = 0
        for t in range(1, max_ep_len + 1):
            action = ppo_agent.select_action(state)
            state, reward, done, _, _, guinfo_copy, LSUPosHistory, odhistoryFull, secrecyrate_info, r_g_s_info, r_g_e_info = env.step1(action,guinfo_copy, LSUPosHistory,odhistoryFull  )
